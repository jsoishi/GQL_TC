...
\begin{figure}
    ...
    \caption{$\theta$-$z$ power spectra of $v$ at mid-gap for $\Reyn_i = 900$ at various GQL cutoffs. Adding a single GQL mode significantly improves the power spectrum. Spiral turbulence appears at $\Lambda=30$. The obvious square in $\Lambda = 60$ represents a sudden drop in spectral power at the cutoff wavenumber. The figure shows only the lowest 100 modes in each direction, though in the simulations $k_{max} = m_{max} = 512$. }
